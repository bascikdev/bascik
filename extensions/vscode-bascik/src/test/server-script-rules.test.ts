...
suite('Server Script Diagnostics', () => {
  suite('Part B: Contract Rules', () => {
    ...
    suite('server-script-legacy-request-shape', () => {
      test('detects req.searchParams, req.headers[, req.path, req.method, and ({ req })', () => {
        const body = `export default async ({ req }) => {
  ...
  return "<p>ok</p>";
        ...
        const diags = analyzeServerScriptSource(body, { hasSrcAttribute: false, directive: 'server' });
        const legacyDiags = diags.filter((d) => d.code === 'server-script-legacy-request-shape');
        assert.strictEqual(legacyDiags.length, 5);

        // Check ({ req })
        assert.ok(legacyDiags[0].message.includes("({ req })"));
        assert.ok(legacyDiags[0].message.includes("(request, context, { signal })"));
        assert.strictEqual(body.slice(legacyDiags[0].start, legacyDiags[0].end), '({ req })');

        // Check req.searchParams
        assert.ok(legacyDiags[1].message.includes("req.searchParams.x"));
        assert.ok(legacyDiags[1].message.includes("new URL(request.url).searchParams.get('x')"));
        assert.strictEqual(body.slice(legacyDiags[1].start, legacyDiags[1].end), 'req.searchParams');

        // Check req.headers[
        assert.ok(legacyDiags[2].message.includes("req.headers['x']"));
        assert.ok(legacyDiags[2].message.includes("request.headers.get('x')"));
        assert.strictEqual(body.slice(legacyDiags[2].start, legacyDiags[2].end), 'req.headers[');

        // Check req.path
        assert.ok(legacyDiags[3].message.includes("req.path"));
        assert.ok(legacyDiags[3].message.includes("new URL(request.url).pathname"));
        assert.strictEqual(body.slice(legacyDiags[3].start, legacyDiags[3].end), 'req.path');

        // Check req.method
        assert.ok(legacyDiags[4].message.includes("req.method"));
        assert.ok(legacyDiags[4].message.includes("request.method"));
        assert.strictEqual(body.slice(legacyDiags[4].start, legacyDiags[4].end), 'req.method');
      });

      test('does not report for standard Request usage', () => {
        const body = `export default async (request, context, { signal }) => {
  ...
  return "<p>ok</p>";
        ...
        const diags = analyzeServerScriptSource(body, { hasSrcAttribute: false, directive: 'server' });
        assert.strictEqual(diags.filter((d) => d.code === 'server-script-legacy-request-shape').length, 0);
      });
    });

    suite('server-script-legacy-output', () => {
      test('detects process.stdout.write, process.env.BASCIK_REQUEST, and top-level console.log', () => {
        const body = `process.stdout.write("hi");
const req = process.env.BASCIK_REQUEST;
...
export default async () => {
  ...
  return "<p>ok</p>";
        ...
        const diags = analyzeServerScriptSource(body, { hasSrcAttribute: false, directive: 'server' });
        const legacyOutDiags = diags.filter((d) => d.code === 'server-script-legacy-output');
        assert.strictEqual(legacyOutDiags.length, 3);
        assert.ok(legacyOutDiags[0].message.includes('Server scripts return their markup from the default export. stdout and console.log are not captured.'));
      });

      test('detects console.log when export default is missing (rule 2 also fires)', () => {
        const body = `console.log("test");`;
        const diags = analyzeServerScriptSource(body, { hasSrcAttribute: false, directive: 'server' });
        const legacyOutDiags = diags.filter((d) => d.code === 'server-script-legacy-output');
        assert.strictEqual(legacyOutDiags.length, 1);
      });

      test('does not report for clean script with console.log only inside export default', () => {
        const body = `export default async (request) => {
  console.log("debug inside handler");
  return "<p>ok</p>";
};`;
        const diags = analyzeServerScriptSource(body, { hasSrcAttribute: false, directive: 'server' });
        assert.strictEqual(diags.filter((d) => d.code === 'server-script-legacy-output').length, 0);
      });
    });
