/**
 * E2E tests for `bascik add` dev server live watching.
 *
 * NOTE: Skipped for static build and the two production server configs (HTTP/1.1 and HTTP/2)
 * because this exercises dynamic component addition during live dev watching.
 *
 * Exercises:
 *   Dev server config: adding a component while the dev server is running and asserting the watcher reloads it.
 */
...
test.describe('bascik add dev watcher', () => {
  ...
  test('dev watcher picks up dynamically added component', async ({ page }) => {
