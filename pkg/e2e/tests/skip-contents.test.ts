/**
 * E2E tests for configured tag-level preserve shielding.
 *
 ...
 * the configured tag's own attributes also remain literal.
 */
...
test.describe('skip-contents-test page', () => {
  ...
  test('the preserved tag itself has literal attributes', async ({ page }) => {
    const codeBlock = page.getByTestId('code-block');

    await expect(codeBlock).toHaveAttribute('class', 'code-block');
    await expect(codeBlock).toHaveAttribute('id', 'code-root');
