...
>
> *Note:* Request-time streaming requires something that executes code per request: the Bascik [dev server](/development-server), the [production server](/production-server), or a serverless function generated by a build `--target` such as Cloudflare Pages (see [Deployment](/deployment#serverless-hosting)). A static-only CDN serving pre-rendered HTML cannot stream anything. For security guidelines when interpolating user data, see [Escaping and injection](/server-scripts#escaping-and-injection).
