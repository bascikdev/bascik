...
- **Go to Definition for Script Imports:** Command-click (or Ctrl-click) a relative import specifier (`import ... from './helper.ts'`, `export ... from`, dynamic `import('./helper.ts')`) or a `src="..."` attribute inside a `<script data-bascik-build>` or `<script data-bascik-server>` tag to jump straight to that file on disk. Bare module specifiers such as `node:fs/promises` and imports in plain client `<script>` tags are not resolved.
