...
| `components.ts` | Loads component HTML and CSS files from disk, detects component tags in HTML strings, extracts props/slots/attributes, and injects resolved content back. Discovery scans each configured components directory (`directory.components` is always an array internally) with byte-wise sorted traversal for deterministic registration, follows symlinks, and throws build errors on duplicate tag collisions across the union of all roots. Tag detection masks `<script>`/`<style>`/`<textarea>` content so literal tag text is never resolved. |
| `component-roots.ts` | Single definition of "which configured components directory owns this file." `findComponentRoot` does a boundary-aware longest-prefix match; every consumer (companion-script layout detection, display paths, `--check`, the exec source-directory warning) calls it instead of testing a single directory string. |
...
- **Build scripts (`<script data-bascik-build>`):** Executed in complete isolation. Instead of using `eval` or Node's `vm` module, which can leak state or restrict standard Node.js APIs, Bascik rewrites relative `./` and `../` ESM imports against the source page or component directory and `@/` imports against the configured import root (`scripts.importRoot`), writes build script content to a temporary `.mjs` file on disk, and runs each uncached script as its OWN standalone Node.js subprocess, bounded by a memory-aware semaphore. There is no shared-process batch: every script gets a fresh ESM module registry, its own stdout/stderr, and its own environment, so output ownership is independent of how many sibling scripts were cache misses. This process-level isolation ensures that user build scripts cannot pollute the memory of the compiler or server, natively supports ES modules, top-level `await`, dynamic imports, and captures stdout cleanly before removing temporary files.
- **Server scripts (`<script data-bascik-server>`):** Executed in-process at request time as Node.js ESM modules via `ScriptRegistry`. During `bascik --build`, the script source is extracted into the sidecar (`dist/.bascik/server-scripts.json`) and replaced in the HTML with an inert placeholder tag (`<script type="text/bascik-server" data-bascik-server-id="...">`). When `bascik --server` boots, it loads the sidecar into memory once. Incoming requests execute the registered module in-process with a WHATWG `Request` and `{ remoteIp }` context and inject the returned markup into the document flow without subprocess spawn overhead.
