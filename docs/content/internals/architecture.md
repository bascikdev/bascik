...
| `build-scripts.ts` | Executes `<script data-bascik-build>` blocks as Node.js ESM modules at transpile time, rewrites relative ESM imports to absolute file URLs, batches uncached scripts per page in a single harness process, caches results on disk, cleans child-process stack traces, and appends sourceURL comments for debugging. |
