...
3. **Parallel Lifecycle Scripts:** All `pipeline.exec` scripts configured with `phase: 'parallel'` launch concurrently with each other and with the steps that follow. In build mode they are joined (awaited) before `dist/` is finalized. In dev mode they are not awaited: the server binds and page compilation starts while they run, and the dev lifecycle owner registers each task's outcome with the exec publication coordinator, which re-transpiles the consuming pages and reloads once on completion or surfaces a build-error on failure. Every started task's rejection is observed in both modes; nothing is fire-and-forget.
