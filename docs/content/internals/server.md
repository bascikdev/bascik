...
**Delivery is fully buffered per request.** Identity (uncompressed) delivery reads the whole file into memory for each request that misses the representation cache; there is no streaming path and no upper bound on the buffered size. Representations at or under `MAX_CACHED_REPRESENTATION_BYTES` (2 MiB) are retained in the bounded cache; anything larger is re-read from disk on every request. Operational guidance: large media (video, large archives, high-resolution image sets) belongs on a CDN or object store, not in `dist/` behind `bascik --server`.

**Precompressed sidecars require a provenance stamp.** A `.br` or `.gz` file next to an asset is served only when a sibling `.bmeta` file records the current raw content hash. Bascik's build does not emit `.bmeta` files today, so pre-existing `.br`/`.gz` sidecars are ignored and the server compresses on the fly instead. If a deployment pipeline produces sidecars, it must also write `<asset>.br.bmeta` / `<asset>.gz.bmeta` containing `{"rawHash": "<quoted content-hash etag>"}` for them to be used.
...
- The import-root and components watchers advance the runtime identity for every changed `.js`/`.ts`/`.mjs` path under their roots before deciding whether any page needs a rebuild. The pages watcher only observes `.html` files, so a script module placed under `directory.pages` is covered only when that directory is nested inside the import root.

**Coverage boundary:** only modules under `scripts.importRoot` and `directory.components` are invalidated this way. A helper that a `src=` server script or API route imports from outside those roots (for example a sibling directory of the import root, or a path reached through `..`) is not invalidated when it changes; the served output does not update until the entry module is edited or the dev server restarts.
