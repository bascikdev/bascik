...
Request-time modules (API routes, `src=` server scripts, and the helpers they import at any depth) are imported by the runtime registry, not by the build, so the page dependency graph knows nothing about them. (Inline `data-bascik-server` scripts are the exception described under Dependency Generations: their helpers are reached through a page recompile, not through a runtime importer edge.) The watchers therefore advance the runtime identity directly:
...
- **Inline `data-bascik-server` scripts.** An inline script is imported as a `data:` URL, and the hook records no edge from a `data:` parent: the helper it imports is tracked (its generation advances on edit, and the watcher logs `module invalidated: src/lib/helper.ts` with no `reloads` list), but no entry is unpublished, so the next request reuses the already-linked inline module and its old helper instance. Transitive helper reload therefore applies to `src=` server scripts and API routes. An inline script picks up the edited helper when its page recompiles, because a changed inline body is a new `data:` URL and resolving the helper from it hands back the helper's advanced generation URL. If nothing recompiles the page, the change lands on restart. Pinned by `dev-module-reload.integration.test.ts`.
