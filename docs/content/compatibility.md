...
| Custom `data-*` attributes | WHATWG HTML §3.2.6.6 | ✓ | Internal directives (`data-bascik-prop-*`, `data-bascik-attr-*`, `data-bascik-slot`, `data-bascik-build`, `data-bascik-routes`, `data-bascik-server`) strictly conform to XML NCName lower-case naming syntax. |
...
| Raw-text content shielding | WHATWG HTML §13.1.2 | ✓ | Internal scans use a shared same-length discard mask, while `scoping.preserve` uses restorable shielding. Multiple preserved tags such as `['pre', 'code']` retain their own content without token collisions. |
