...
| `<script data-bascik-server>`                            | ✓     | Executed on the server at **request time** in-process as a Node.js ESM module via `ScriptRegistry`. Returned markup replaces the tag dynamically. Receives `{ req }` and `{ signal }`. Top-level `import`, `await`, and `escapeHtml` are supported. |
