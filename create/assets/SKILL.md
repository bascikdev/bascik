...
| `BASCIK_OUT_DIR` | Absolute path to the configured output directory (exec scripts). |
...
bascik add <package>          # copy-in components from an installed npm package into src/components/
```

More flags: `--config <path>` (load a specific config file), `--port <n>`, `--host <name>`, `--log-level <silent|error|warn|info|debug>`, `--site-url <url>`, `--only <glob>` (repeatable, only with `--build`), `--force` (overwrite locally modified components during `add`), `--dry-run` (preview what `add` would copy), `--yes`/`-y` (skip interactive prompts during `add`), and `--env-file <path>` (repeatable). Every value flag accepts both `--flag value` and `--flag=value`. Precedence: a flag beats the environment variable (`BASCIK_SERVER_PORT`, `BASCIK_SERVER_HOST`, `BASCIK_LOG_LEVEL`), which beats the config file. The parser rejects `--build` combined with `--server`, `--only` without `--build`, unknown flags, and stray positionals: `bascik build` (no dashes) errors with a `Did you mean "--build"?` suggestion instead of silently starting the dev server.

**`bascik add`:** copies components from an installed npm package into `src/components/`. Files are **project files** committed to version control, not resolved runtime dependencies. Component tag names derive from filenames; if a package component collides with an existing project component, `bascik add` refuses to write anything and reports both paths. `bascik add` maintains a `bascik-lock.json` file in the project root with file hashes. Unmodified files update cleanly on re-add; locally modified files are never overwritten without `--force`.
...
**`bascik --build --target <name>`:** also emits a serverless deployment bundle under `dist/.bascik/<name>/` (`cloudflare`, `cloudflare-workers`, or `cloudflare-pages`). The normal `dist/` is unchanged. The bundle has a `public/` upload tree (static files plus the generated Worker and routing) and compiles every server script, stream script, API route, and dynamic page template into the Worker; dynamic templates are never in `public/`, so a routing miss is a 404, not inert placeholders. Official targets resolve to `@bascik/adapter-cloudflare` (`npm install --save-dev @bascik/adapter-cloudflare`). Rejected with `--only`. Unsupported `node:*` imports (for example `node:child_process`) fail the build with the import chain. Streamed pages are sent with `Content-Encoding: identity` so the edge does not buffer them. See the Cloudflare deployment guide.

**`bascik --server`:** starts the production server against a pre-built `dist/` directory (HTTP/1.1 by default; HTTP/2 when TLS is enabled). **Needed when the site uses `data-bascik-server` or `data-bascik-stream` scripts or API routes (`src/api/`)** for per-request dynamic content, unless you deploy a serverless bundle instead. Sites with only static pages and none of those can be deployed to any static host. Run `bascik --build` first, then `bascik --server`. Unlike the dev server, `--server` does not watch files or inject live-reload. `data-bascik-server` scripts and API routes execute per-request in both modes.
...
* **Portability:** The handler function is reusable because it takes a standard `Request`, but routing, method dispatch, and imports still need a host. Bascik supplies all three on `bascik --server` and on the Cloudflare target (`bascik --build --target cloudflare|cloudflare-workers|cloudflare-pages`, see below). Other hosts are manual porting: write the platform wrapper yourself.
* **`context.platform`:** optional host capabilities. `{ name: "node" }` on the built-in server; `{ name: "cloudflare", env, waitUntil }` in a generated Worker, where `env` carries KV/D1/secrets bindings. Never copied to globals; a handler that ignores it runs unchanged everywhere.
* **Gotchas:** Plain static builds (`bascik --build`) cannot serve API routes and warn at build time; add `--target <name>` to package them into a serverless bundle. Handlers run in-process on the server; synchronous infinite loops cannot be interrupted by timers. No automatic compression or CORS headers are added by default.
...
**`http.compression`:** negotiates Brotli or Gzip based on `Accept-Encoding`. Static HTML pages are pre-compressed with both Brotli and Gzip in the background at startup; static assets negotiate Brotli or Gzip with sidecar support (`.br`/`.gz`). Compression is skipped below a size threshold and for already-compressed formats (images, video, audio, archives, `woff2`). Each encoded representation gets a distinct ETag from the identity representation (e.g. `"hash-br"`, `"hash-gz"`).
